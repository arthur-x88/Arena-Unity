...
namespace Core
{
    internal class WarcraftCharacterController : EntityBehaviour<IUnitState>, IUnitBehaviour
    {
        ...
        {
            // Ensure Rigidbody settings are up to date before applying movement.
            UpdateRigidbody();
            ...
            if (!unitRigidbody.isKinematic && unitRigidbody.velocity.y > controllerDefinition.JumpSpeed)
                unitRigidbody.velocity = new Vector3(unitRigidbody.velocity.x, controllerDefinition.JumpSpeed, unitRigidbody.velocity.z);

            ProcessGroundState();

            if (!unitRigidbody.isKinematic && unit.SlowFallSpeed != 0 && unit.HasMovementFlag(MovementFlags.Flying) && unitRigidbody.velocity.y < -unit.SlowFallSpeed)
